...
class splitbase():
    def __init__(self,
        ...
        self.gap_w = gap_w
        self.gap_h = gap_h
        self.subsize_w = subsize_w
        self.subsize_h = subsize_h
        self.slide_w = self.subsize_w - self.gap_w
        self.slide_h = self.subsize_h - self.gap_h
        self.thresh = thresh
    ...
    def saveimagepatches(self, img, subimgname, left, up):
        subimg = copy.deepcopy(img[up: (up + self.subsize_h), left: (left + self.subsize_w)])
    ...
    def SplitSingle(self, name, rate, extent):
        ...
        if resizeimg.shape[0]<self.subsize_h:
            ...
            resizeimg = cv2.copyMakeBorder(resizeimg,0,self.subsize_h-resizeimg.shape[0],0,0,cv2.BORDER_CONSTANT,value=0)
        if resizeimg.shape[1]<self.subsize_w:
            resizeimg = cv2.copyMakeBorder(resizeimg,0,0,0,self.subsize_w-resizeimg.shape[1],cv2.BORDER_CONSTANT,value=0)
